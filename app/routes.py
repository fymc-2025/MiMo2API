...
async def chat_completions(
    ...
    if request.audio and ('-tts' in effective_model or '-voicedesign' in effective_model):
        acct_tts = await _get_tts_account()
        voice_name = request.audio.get("voice", "alloy")
        user_content = ""
        synth_text = ""
...
async def _tts_generate(
    ...
    cookie_str = f'userId={acct.user_id}; serviceToken="{acct.service_token}"; xiaomichatbot_ph="{ph}"'
...
# ═══════════════════════════════════════════════════════════════
# OpenAI Responses API 兼容层（无工具调用版）
# 追加到 /v1/audio/speech 路由之后
# ═══════════════════════════════════════════════════════════════
...
def _strip_tool_result_blocks(text: str) -> str:
    """移除 MiMo 模型输出中可能残留的 tool_result 块。"""
    if not text:
        return text
    return re.sub(r'<tool_result>.*?</tool_result>', '', text, flags=re.DOTALL).strip()
...
async def _do_response_chat(body: dict, account) -> tuple:
    ...
    # 转换 dict 消息为 OpenAIMessage 对象（无 tool_calls 字段）
    openai_messages = []
    for m in messages:
    ...
    # 构建查询（无工具）
    query = build_query_from_messages(openai_messages)
    ...
    # 清理输出（仅 strip tool_result 块和引用标记，无工具提取）
...
# ─── 核心函数：流式（无工具、无 StreamSieve）─────────────────

async def _stream_response_events(body: dict, account):
    """流式 Responses API 事件生成器（无工具版 — 仅缓冲区 + think 标签解析）。
    ...
    try:
        # 简单流式 — 仅缓冲区 + think 标签解析，无 StreamSieve，无工具路径
        in_think = False
        buffer = ""
