...
支持流式、深度思考、系统指令等，详见下方 [Responses API 详解](#responses-api-详解)。
...
MiMo2API 完整实现了 OpenAI Responses API 格式（no-tools 分支仅支持纯对话，不含工具调用）。
...
```json
{
  "output": [
    ...
    },
    {
      ...
      "role": "assistant",
      "status": "completed",
      "content": [{"type": "output_text", "text": "你好！有什么可以帮你的？"}]
    }
  ]
}
```
